...
- The `SessionStart` block lives in three copies: `hooks/hooks.json`, the hand-install snippet in the README, and `demo/homelab/.claude/settings.json`. Cases 5, 12 and 13 of `tests/manifests.sh` hold them to one shape, so they move together or CI says so.
