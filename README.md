...
That's the demo's handoff, and `./demo/setup.sh` puts it in a project you can open. Around 300 to 500 tokens is the useful range, shorter usually hides a vague next action and longer turns into a journal.
...
2. It's read automatically at every session start. If a human has to remember to load it, it gets forgotten on the session where it mattered.
3. The agent writes it at milestones, unprompted. A decision made, a PR opened or merged, an investigation concluded, a blocker hit, a question left with a human. Not after every message.
...
5. Every update removes what's resolved. A handoff that only grows stops being read.
6. Empty is fine. When nothing is left in flight, the file gets deleted.

Point 6 is the first one people drop, and the FAQ says why it matters.

## Where the handoff lives

An agent running in `/home/alice/work/acme/api` reads `~/.claude/handoffs/work-acme-api-32817b.md` by default. That's the working directory relative to your home, separators turned into dashes, and a short digest of the full path on the end. Run `session-start-handoff.sh --path` in a directory and it prints that name, no need to work it out.

An agent that isn't tied to one directory pins `PERSISTENT_HANDOFF_FILE` to an absolute path instead. There's one expansion trap in doing that, and the digest guards against a coincidence, it doesn't prove uniqueness. [`docs/REFERENCE.md`](docs/REFERENCE.md) has both, plus what happens when several sessions share one path and what Claude Code's 10,000 character cap on hook output does to a handoff grown into a journal.

## FAQ

**Why not just use `/compact`?** A compaction summary lives in the session that holds it and dies with it. The file survives a crash, a `kill -9` and a reboot, none of which give the model a chance to summarize anything. They also hold different material. Compaction keeps what the model judged worth keeping. The handoff keeps what you decided matters, in your words. Use both.

**What happens after a compact or a resume?** The hook fires there too, about 500 tokens, so the handoff is present whatever the compaction summary happened to keep, and nothing the summary dropped is lost. The preamble says which side wins. Your context is newer for what you did in this session, the file is the reference for everything else. If the two disagree, update the file. Repeated resumes don't stack copies, for the reason [`docs/REFERENCE.md`](docs/REFERENCE.md) spells out.

**Why delete the file when it's empty?** Because every session start reads it. A handoff that's always there, half stale, describing work that finished last week, teaches every new session to skim it. Deleting it keeps the file meaningful. If it's there, something is in flight. For an agent whose work never ends, and so never gets to delete it, pruning every update does the same job.

**Why not just keep this in CLAUDE.md or the agent's memory?** Different lifetimes. CLAUDE.md and memory hold what stays true, like mechanisms, commands and preferences. The handoff holds what's in flight right now, and gets deleted when it lands. A stable fact in the handoff dies with the work, and a current blocker in memory outlives its own fix.
...
**What does it cost on every session start?** Between 10 and 25 ms, depending on the machine and what else it's doing. Zero tokens when no handoff exists, which is the normal case. With one, roughly 400 to 600 tokens.
