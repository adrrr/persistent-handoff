...
5. Check `actions/checkout` in `.github/workflows/tests.yml` is still pinned to a SHA, and refresh the pin if it is behind. There is no dependabot on this repo on purpose: one action, and a bump nobody reads is worse than a line on this list.
6. Merge. That is the moment the release goes out to installers, before the tag exists.
